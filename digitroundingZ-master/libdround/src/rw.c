...
void writeData(void *data, int dataType, size_t nbEle, char *tgtFilePath, int *status)
{
	...
	{
		printf("Error: data type cannot be the types other than DROUND_FLOAT or DROUND_DOUBLE\n");
		*status = DROUND_TERR; //wrong type
		return;
	}
	*status = state;
}
