...
int main(int argc, char** argv) {
  ...
  if(argc < 2)
  {
    printf("Test case: ./mycompress_bitnp_double [input_txt]\n");
    printf("Example: ./mycompress_bitnp_double input.txt\n");
    exit(0);
  }
  ...
  myCompress_bitwise_double_np(data_small, num, &data_bits_np, &bytes_np, &pos_np);
  ...
  double* decompressed_data = myDecompress_bitwise_double_np(data_bits_decomp, bytes_decomp, num);
  ...
  float compress_ratio = (float)(bytes_np*8)/(num*sizeof(double)*8);
